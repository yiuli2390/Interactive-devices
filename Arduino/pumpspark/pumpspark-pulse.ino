...
// state[i] is (inverse of) the setting of pins[i], which controls pumps[i]
      boolean  state[]       = {  0,  0,  0};
 long int      timers[]      = {  0,  0,  0};
      boolean  pumpOn[]        = {  0,  0,  0};
...
void loop() { // run over and over
    ...
    progressSequence();
    printIntArray(timers, 3);
    delay(500);    // Neccessary 10ms delay because using the switches sometimes causes "flickering"
}
...
void driveState()
{
    for (int i = 0; i < 3; i++)
    {
        Serial.print("State ");Serial.print(state[i]);Serial.print(" pumpOn ");Serial.print(pumpOn[i]); Serial.print(" millis ");Serial.println(millis());
        if (state[i] && !pumpOn[i] && ((millis() - timers[i]) > 3000))
        {
            timers[i] = millis();
            on(continPumps[i]);
            pumpOn[i] = 1;
        ...
        else if (state[i] && ((millis() - timers[i]) > 3000))
        {
            timers[i] = millis();
            off(continPumps[i]);
            pumpOn[i] = 0;
...
boolean updateState()
{
    ...
    if (isOn != state[0])
    {
        Serial.println("lightPin Changed");
        stateChanged = true;
        timers[0] = millis();
        if(isOn)
        {
    ...
    if (isOn != state[1])
    {   
        Serial.println("screenPin Changed"); 
        stateChanged = true;
        timers[1] = millis();
        if(isOn)
        {
...
void printIntArray(long int* arr, int size)
{
    String message = "[ ";
